...
__device__ __forceinline__ float3 boost(float3 b, float3 vec, float gamma)
{
    ...
    if (len2 < 1e-12f) {
        return vec; // 速度为0或极小时，避免除以0，直接返回原向量
    }
...
blackholekernel(float4 *__restrict__ raw_img, cudaTextureObject_t tex_obj, cudaTextureObject_t lut_physics,
...
)
{
    ...
    for (int i = 0; i < jitternum; ++i) {
        ...
        // jitterx = rand_float((unsigned
        // int)pixel_idx+frames*1919810+i); jittery =
        // rand_float((unsigned int)pixel_idy+12345*i+frames*114514);
        ...
        // float3 tmp1 =
        // normalize(make_float3(focal_length,-physical_x,-physical_y));

        float r_pixel = sqrtf(physical_x * physical_x + physical_y * physical_y); // 鱼眼镜头
        ...
        // float depth_jitter = rand_float((unsigned int)pixel_idx +
        // (unsigned int)pixel_idy * 114 + i*514);
        ...
        cam_pos = cam_pos + d * (depth_jitter * r / 10.0f * step); // 注意这里用基准 step
                                                                   // 即可，防止自适应步长尚未计算
        float3 p = d * n;
        float3 p_init = p;
        ...
        for (int s = 0; s < maxstep && flag; ++s) {
            ...
            // in_disk_volume 为 true 时(1.0)，应用
            // 0.05f，否则为 1.0f
            ...
            // float r_disk = sqrtf(cam_pos.x * cam_pos.x +
            // cam_pos.y * cam_pos.y);
            ...
            if (indisk) {
                ...
                float4 parameters = tex2D<float4>(lut_physics, (r_disk - 4.9495f) / 10.0505f, fabsf(cam_pos.z) / 2.5f);

                // calculate g factor
                float td, pd;
                tdpd(r, &td, &pd);

                // float phi = atan2f(temp.y,temp.x);
                // float rot = fmodf(pd * time /
                // td, 6.283185307179586f); phi += rot; float g
                // = fmaxf((fabsf((factor*gamma+p_init*e0)
                // /(td-pd*lz))-1.0f)*1.0f+1.0f,0.01f); float
                // s,c;sincosf(phi,&s,&c);
                float inv_r_disk = rsqrtf(r_disk_sq); // 快速求倒数
                float g = fmaxf((fabsf((factor * gamma + p_init * e0) / (td - pd * lz)) - 1.0f) * 1.0f + 1.0f, 0.01f);
                float cos_phi0 = temp.x * inv_r_disk;
                float sin_phi0 = temp.y * inv_r_disk;
                float sin_rot, cos_rot;
                float rot = fmodf(pd * time / td, 6.283185307179586f);
                sincosf(rot, &sin_rot, &cos_rot);

                float c = cos_phi0 * cos_rot - sin_phi0 * sin_rot;
                float s = sin_phi0 * cos_rot + cos_phi0 * sin_rot;
                float3 pos = make_float3(r * c, r * s, cam_pos.z * 5.0f);
                // g = 1.0f;
                // float noise = 1.0f;
                float noise = (cudaNoise::repeaterTurbulence(pos, 0.3f, 1.5f, 114514, 0.25f, 2,
                                                             cudaNoise::BASIS_SIMPLEX, cudaNoise::BASIS_SIMPLEX) -
                               0.0f) *
                              0.5f; /*这个参数已经
// 调好了，不要轻易改*/
                // float noise =
                // (cudaNoise::repeaterTurbulence(pos,4.5f,0.9f,114514,0.30f,3,cudaNoise::BASIS_SIMPLEX,cudaNoise::BASIS_SIMPLEX)-0.0f)
                // * 0.7f;
                parameters.x = parameters.x * __expf(noise);
                // parameters.x = fmaxf(parameters.x,0.2f);
                parameters.y = parameters.y * __expf(noise * 0.67f);
                parameters.z = parameters.z * __expf(noise * 2.67f);
                float4 emission =
                    disk_emission(fmaxf(parameters.y * g, 1000.0f), parameters.z * g * g * g * g, lut_color);
                // float4 emission =
                // disk_emission(19000,parameters.z*g*g*g*g,lut_color);
                ...
                float intensity_factor =
                    1.0f - __expf(-(k * parameters.z * g * g * g * g) * (k * parameters.z * g * g * g * g));
                // float intensity_factor = 1.0f - __expf(-(k *
                // parameters.z)*(k * parameters.z));
                float step_opacity =
                    parameters.x * 1.7f * uuu * uuu * length(cam_pos - prev_pos) * intensity_factor / g;
                // step_opacity = parameters.y*g<1500.0f ? 0.0f
                // : fminf(step_opacity, 1.0f); step_opacity =
                // fminf(step_opacity, 1.0f);
                float temp_fade = __saturatef((parameters.y * g - 1400.0f) / 500.0f);
                // step_opacity = fminf(step_opacity *
                // temp_fade, 1.0f);
...
extern "C" __global__ void taaColorClampingKernel(const float4 *currentFrame, // 当前帧（带 Jitter 渲染出来的图）
                                                  const float4 *prevFrame,    // 上一帧的输出图
                                                  float4 *outputFrame, // 最终输出图（同时作为下一帧的 prevFrame）
                                                  int width, int height,
                                                  float alpha, // 混合系数，建议 0.1 ~ 0.2
                                                  int frames)
{
    ...
    // 1. 统计当前像素周围 3x3 邻域的 RGB 颜色范围（包围盒）
    ...
    final_col.w = curr_col.w; // 保持透明度通道
