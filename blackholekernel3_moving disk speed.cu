...
// jitterx = rand_float((unsigned int)pixel_idx+frames*1919810+i);
// jittery = rand_float((unsigned int)pixel_idy+12345*i+frames*114514);
...
// float depth_jitter = rand_float((unsigned int)pixel_idx + (unsigned int)pixel_idy * 114 + i*514);
...
cam_pos = cam_pos + d * (depth_jitter *r/10.0f * step); // 注意这里用基准 step 即可，防止自适应步长尚未计算
float3 p = d * n;
float3 p_init = p;
...
if (indisk) {
    ...
    // g = 1.0f;
    //float noise = 1.0f;
    float noise = (cudaNoise::repeaterTurbulence(pos,0.3f,1.5f,114514,0.25f,2,cudaNoise::BASIS_SIMPLEX,cudaNoise::BASIS_SIMPLEX)-0.0f) * 0.5f;/*这个参数已经
    //                                                                                                                                     调好了，不要轻易改*/
    //float noise = (cudaNoise::repeaterTurbulence(pos,4.5f,0.9f,114514,0.30f,3,cudaNoise::BASIS_SIMPLEX,cudaNoise::BASIS_SIMPLEX)-0.0f) * 0.7f;                                                                                                                                     
    ...
    // step_opacity = fminf(step_opacity * temp_fade, 1.0f);
...
__global__ void taaColorClampingKernel(
    const float4* currentFrame, // 当前帧（带 Jitter 渲染出来的图）
    ...
    // 3. 将上一帧颜色强行限制在当前帧的 3x3 范围内（这是消除重影的魔法喵！）
    float3 clamped_prev;
