...
// Shaping functions
typedef enum { SHAPE_STEP, SHAPE_LINEAR, SHAPE_QUADRATIC } profileShape;

// Function blending operators
typedef enum { OPERATOR_ADD, OPERATOR_AVG, OPERATOR_MUL, OPERATOR_MAX, OPERATOR_MIN } repeatOperator;
...
__device__ float3 vectorNoise(int x, int y, int z)
{
    return make_float3(randomFloat(x * 8231.0f + y * 34612.0f + z * 11836.0f + 19283.0f) * 2.0f - 1.0f,
                       randomFloat(x * 1171.0f + y * 9234.0f + z * 992903.0f + 1466.0f) * 2.0f - 1.0f, 0.0f);
}
...
// Random spots
__device__ float spots(float3 pos, float scale, int seed, float size, int minNum, int maxNum, float jitter,
                       profileShape shape)
{
    if (size < EPSILON)
        return 0.0f;
    ...
    for (int x = -1; x < 2; x++) {
        for (int y = -1; y < 2; y++) {
            for (int z = -1; z < 2; z++) {
                int numSpots = randomIntRange(minNum, maxNum,
                                              seed + (ix + x) * 823746.0f + (iy + y) * 12306.0f + (iz + z) * 823452.0f +
                                                  3234874.0f);

                for (int i = 0; i < numSpots; i++) {
                    float distU = u - x -
                                  (randomFloat(seed + (ix + x) * 23784.0f + (iy + y) * 9183.0f +
                                               (iz + z) * 23874.0f * i + 27432.0f) *
                                       jitter -
                                   jitter / 2.0f);
                    float distV = v - y -
                                  (randomFloat(seed + (ix + x) * 12743.0f + (iy + y) * 45191.0f +
                                               (iz + z) * 144421.0f * i + 76671.0f) *
                                       jitter -
                                   jitter / 2.0f);
                    float distW = w - z -
                                  (randomFloat(seed + (ix + x) * 82734.0f + (iy + y) * 900213.0f +
                                               (iz + z) * 443241.0f * i + 199823.0f) *
                                       jitter -
                                   jitter / 2.0f);
...
__device__ float worleyNoise(float3 pos, float scale, int seed, float size, int minNum, int maxNum, float jitter)
{
    ...
    for (int x = -1; x < 2; x++) {
        for (int y = -1; y < 2; y++) {
            for (int z = -1; z < 2; z++) {
                int numPoints = randomIntRange(minNum, maxNum,
                                               seed + (ix + x) * 823746.0f + (iy + y) * 12306.0f + (iz + z) * 67262.0f);
...
__device__ float tricubic(int x, int y, int z, float u, float v, float w)
{
    ...
    float x00 = cubic(randomGrid(x - 1, y - 1, z - 1), randomGrid(x, y - 1, z - 1), randomGrid(x + 1, y - 1, z - 1),
                      randomGrid(x + 2, y - 1, z - 1), u);
    float x01 = cubic(randomGrid(x - 1, y - 1, z), randomGrid(x, y - 1, z), randomGrid(x + 1, y - 1, z),
                      randomGrid(x + 2, y - 1, z), u);
    float x02 = cubic(randomGrid(x - 1, y - 1, z + 1), randomGrid(x, y - 1, z + 1), randomGrid(x + 1, y - 1, z + 1),
                      randomGrid(x + 2, y - 1, z + 1), u);
    float x03 = cubic(randomGrid(x - 1, y - 1, z + 2), randomGrid(x, y - 1, z + 2), randomGrid(x + 1, y - 1, z + 2),
                      randomGrid(x + 2, y - 1, z + 2), u);

    float x10 = cubic(randomGrid(x - 1, y, z - 1), randomGrid(x, y, z - 1), randomGrid(x + 1, y, z - 1),
                      randomGrid(x + 2, y, z - 1), u);
    float x11 =
        cubic(randomGrid(x - 1, y, z), randomGrid(x, y, z), randomGrid(x + 1, y, z), randomGrid(x + 2, y, z), u);
    float x12 = cubic(randomGrid(x - 1, y, z + 1), randomGrid(x, y, z + 1), randomGrid(x + 1, y, z + 1),
                      randomGrid(x + 2, y, z + 1), u);
    float x13 = cubic(randomGrid(x - 1, y, z + 2), randomGrid(x, y, z + 2), randomGrid(x + 1, y, z + 2),
                      randomGrid(x + 2, y, z + 2), u);

    float x20 = cubic(randomGrid(x - 1, y + 1, z - 1), randomGrid(x, y + 1, z - 1), randomGrid(x + 1, y + 1, z - 1),
                      randomGrid(x + 2, y + 1, z - 1), u);
    float x21 = cubic(randomGrid(x - 1, y + 1, z), randomGrid(x, y + 1, z), randomGrid(x + 1, y + 1, z),
                      randomGrid(x + 2, y + 1, z), u);
    float x22 = cubic(randomGrid(x - 1, y + 1, z + 1), randomGrid(x, y + 1, z + 1), randomGrid(x + 1, y + 1, z + 1),
                      randomGrid(x + 2, y + 1, z + 1), u);
    float x23 = cubic(randomGrid(x - 1, y + 1, z + 2), randomGrid(x, y + 1, z + 2), randomGrid(x + 1, y + 1, z + 2),
                      randomGrid(x + 2, y + 1, z + 2), u);

    float x30 = cubic(randomGrid(x - 1, y + 2, z - 1), randomGrid(x, y + 2, z - 1), randomGrid(x + 1, y + 2, z - 1),
                      randomGrid(x + 2, y + 2, z - 1), u);
    float x31 = cubic(randomGrid(x - 1, y + 2, z), randomGrid(x, y + 2, z), randomGrid(x + 1, y + 2, z),
                      randomGrid(x + 2, y + 2, z), u);
    float x32 = cubic(randomGrid(x - 1, y + 2, z + 1), randomGrid(x, y + 2, z + 1), randomGrid(x + 1, y + 2, z + 1),
                      randomGrid(x + 2, y + 2, z + 1), u);
    float x33 = cubic(randomGrid(x - 1, y + 2, z + 2), randomGrid(x, y + 2, z + 2), randomGrid(x + 1, y + 2, z + 2),
                      randomGrid(x + 2, y + 2, z + 2), u);
...
// Fast function for fBm using perlin noise
__device__ float repeaterPerlinBounded(float3 pos, float scale, int seed, int n, float lacunarity, float decay,
                                       float threshold)
{
    float acc = 1.0f;
    float amp = 1.0f;
    ...
    for (int i = 0; i < n; i++) {
        acc *= 1.0f - __saturatef(0.5f + 0.5f * perlinNoise(make_float3(pos.x * scale, pos.y * scale, pos.z * scale),
                                                            1.0f, seed ^ ((i + 38) * 27389482))) *
                          amp;
...
__device__ float repeaterSimplexBounded(float3 pos, float scale, int seed, int n, float lacunarity, float decay,
                                        float threshold)
{
    float acc = 1.0f;
    float amp = 1.0f;
    ...
    for (int i = 0; i < n; i++) {
        float val = __saturatef((simplexNoise(make_float3(pos.x * scale + 32240.7922f, pos.y * scale + 835622.882f,
...
__device__ float repeater(float3 pos, float scale, int seed, int n, float lacunarity, float decay, basisFunction basis)
{
    ...
    for (int i = 0; i < n; i++) {
        switch (basis) {
        case (BASIS_CHECKER):
            acc += checker(make_float3(pos.x * scale + 53872.1923f, pos.y * scale + 58334.4081f,
                                       pos.z * scale + 9358.34667f),
                           1.0f, seed) *
                   amp;
            break;
        case (BASIS_DISCRETE):
            acc += discreteNoise(make_float3(pos.x * scale + 7852.53114f, pos.y * scale + 319739.059f,
                                             pos.z * scale + 451336.504f),
                                 1.0f, seed) *
                   amp;
            break;
        case (BASIS_LINEARVALUE):
            acc += linearValue(make_float3(pos.x * scale + 940.748139f, pos.y * scale + 10196.4500f,
                                           pos.z * scale + 25650.9789f),
                               1.0f, seed) *
                   amp;
            break;
        case (BASIS_FADEDVALUE):
            acc += fadedValue(make_float3(pos.x * scale + 7683.26428f, pos.y * scale + 2417.78195f,
                                          pos.z * scale + 93889.4897f),
                              1.0f, seed) *
                   amp;
            break;
        case (BASIS_CUBICVALUE):
            acc += cubicValue(make_float3(pos.x * scale + 6546.80178f, pos.y * scale + 14459.4682f,
                                          pos.z * scale + 11616.5811f),
                              1.0f, seed) *
                   amp;
            break;
        case (BASIS_PERLIN):
            acc += perlinNoise(make_float3(pos.x * scale + 1764.66931f, pos.y * scale + 2593.55017f,
                                           pos.z * scale + 4813.24412f),
                               1.0f, seed) *
                   amp;
            break;
        case (BASIS_SIMPLEX):
            acc += simplexNoise(make_float3(pos.x * scale + 7442.93020f, pos.y * scale + 8341.06698f,
                                            pos.z * scale + 66848.7870f),
                                1.0f, seed) *
                   amp;
            break;
        case (BASIS_WORLEY):
            acc += worleyNoise(make_float3(pos.x * scale + 7619.01285f, pos.y * scale + 57209.0681f,
                                           pos.z * scale + 1167.91397f),
                               1.0f, seed, 0.1f, 4, 4, 1.0f) *
                   amp;
            break;
        case (BASIS_SPOTS):
            acc += spots(make_float3(pos.x * scale + 33836.4116f, pos.y * scale + 2242.51045f,
...
__device__ float fractalSimplex(float3 pos, float scale, int seed, float du, int n, float lacunarity, float decay)
{
    ...
    for (int i = 0; i < n; i++) {
        acc += simplexNoise(
                   make_float3(pos.x * scale + 617.437379f, pos.y * scale + 196410.219f, pos.z * scale + 321280.627f),
                   1.0f, seed * (i + 1)) *
               amp;
        scale *= lacunarity;
        amp *= decay;
...
__device__ float turbulence(float3 pos, float scaleIn, float scaleOut, int seed, float strength, basisFunction inFunc,
...
{
    pos.x +=
        (repeater(make_float3(pos.x, pos.y, pos.z), scaleIn, seed ^ 0x41728394, n, 2.0f, 0.5f, basisIn)) * strength;
    pos.y +=
        (repeater(make_float3(pos.x, pos.y, pos.z), scaleIn, seed ^ 0x72837263, n, 2.0f, 0.5f, basisIn)) * strength;
    pos.z +=
        (repeater(make_float3(pos.x, pos.y, pos.z), scaleIn, seed ^ 0x26837363, n, 2.0f, 0.5f, basisIn)) * strength;
