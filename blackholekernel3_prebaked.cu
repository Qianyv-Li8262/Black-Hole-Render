    ...
    for (int i = 0; i < jitternum; ++i) {
        ...
        for (int s = 0; s < maxstep && flag; ++s) {
            ...
            if (indisk) {
                ...
                // Independent ALU work while tex3D is in flight
                // — hides scoreboard stall
                ...
                // Now parameters are ready — compute everything
                // that depends on parameters but NOT on the
                // lut_color texture (do it before second TEX to
                // hide its latency too)
                float k = 2.0f;
                ...
                // Second TEX: lut_color lookup — only now,
                // after hiding tex3D latency
                float4 emission = disk_emission_dep(fmaxf(parameters.y * g, 1000.0f), parameters.z * g4, lut_color);
                // float4 emission =
                // disk_emission(fmaxf(parameters.y * g,
                // 1000.0f), parameters.z * g4);
                //  accumulated_color.x += emission.x * alpha *
                //  transmittance; accumulated_color.y +=
                //  emission.y * alpha * transmittance;
                //  accumulated_color.z += emission.z * alpha *
                //  transmittance; accumulated_color.w += alpha
                //  * transmittance;
