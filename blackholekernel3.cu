...
extern "C" __global__ void blackholekernel(float4 *__restrict__ raw_img, cudaTextureObject_t tex_obj,
                                           cudaTextureObject_t lut_physics, cudaTextureObject_t lut_color,
                                           const float cam_pos_x, const float cam_pos_y, const float cam_pos_z,
                                           const float fwd_x, const float fwd_y, const float fwd_z, const float right_x,
                                           const float right_y, const float right_z, const float up_x, const float up_y,
                                           const float up_z, const int imgwidth, const int imgheight,
                                           const float physwidth, const float physheight, const float focal_length,
                                           const float step, const int maxstep, const int jitternum, const int frames
...
)
{
    ...
    float3 d = make_float3(fwd.x * focal_length - right.x * physical_x - up.x * physical_y,
    ...
    for (int s = 0; s < maxstep && flag; ++s) {
        ...
        // float current_step = step * fminf(10.0f, fmaxf(0.05f, (r -
        // 0.95f)));
        ...
        // float r_disk = sqrtf(cam_pos.x * cam_pos.x + cam_pos.y *
        // cam_pos.y);
